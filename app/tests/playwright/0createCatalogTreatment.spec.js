...
const config = require('./config');
 ...
 test('create catalog treatments', async ({ page }) => {
    ...
    // examinations a git
     ...
     await page.waitForSelector('a[href="/catalogs/treatments/"]', { state: 'visible' });
     await page.click('a[href="/catalogs/treatments/"]');

    await expect(page).toHaveURL(config.urls.CatalogTreatment);
    ...
    await expect(newTab).toHaveURL(config.urls.CreateCatalogTreatments);


        // treatment name
    await newTab.waitForSelector('#id_treatment_name', { visible: true, timeout: 5000 });
    await newTab.fill('#id_treatment_name', 'John Doe');
    ...
    await newTab.click('.select2-results__option:has-text("John Doe")'); // Seç
      ...
      //treatment  description
        ...
        await newTab.fill('#id_description', 'John Doe catalog treatment description');
