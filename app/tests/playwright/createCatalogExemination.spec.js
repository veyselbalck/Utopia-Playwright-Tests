...
const config = require('./config');
 ...
 test('create of examinations', async ({ page }) => {
    ...
    await expect(newTab).toHaveURL(config.urls.CreateExaminationPage);


    // Muayene bilgilerini doldur
    // Examination  name
await newTab.waitForSelector('#id_examination_name', { visible: true, timeout: 5000 });
await newTab.fill('#id_examination_name', 'John Doe');
